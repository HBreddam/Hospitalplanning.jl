...
function setup_sub!(subproblems,patients::Array{Patient},resources::Array{Resource},mastercalendar::Dict{Int,Date},TimeDelta)
    for patient in patients

        #TODO length of min treatment time should be calculated and used.


        setup_sub!(subproblems,patient,resources,mastercalendar,TimeDelta)
    end
end


function setup_sub!(subproblems,patient::Patient,resources::Array{Resource},mastercalendar::Dict{Int,Date},TimeDelta)
    M1 = 10
    M2 = 1000
    ...
    startdate = (bestordmin-Month(1))-Day(bestordmin-Month(1))+Day(1)
    enddate = max(bestordmax+Month(2)-Day(bestordmax+Month(2)), (bestordmin+Day(maxwait)+Month(1)-Day(bestordmin+Day(maxwait)+Month(1))))
    ...
    if !haskey(subproblems.pricingproblems,sets.hash)
        ...
        @objective(sub,Min, 0)
            ...
            #     sum(Ts[d][j2][i]*xvars[v2,d,j2,i] for d in D_v[v2],j2 in J_d[d],i in I[d][j2] )
            #     - sum(Te[d][j2][i]*xvars[v1,d,j2,i] for d in D_v[v1],j2 in J_d[d],i in I[d][j2])
            #     <=  kvars[v1,v2]*M2)
